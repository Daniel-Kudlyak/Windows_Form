...
namespace IZP
{
    public partial class Головне_меню_вівторок : Form
    {
        public Головне_меню_вівторок()
        {
            InitializeComponent();
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command_Mon = new MySqlCommand("SELECT  `Tuesday` FROM `users` WHERE `login` = @loginUser AND `pass` = @passUser ", baza_Mon.GetConnection());
            ...
            String res_Tue = string.Join(Environment.NewLine, table_Mon.Rows.OfType<DataRow>().Select(x => string.Join(";", x.ItemArray)));
            Авторизація.Tue = Convert.ToInt32(res_Tue);

            Авторизація.Tue = Авторизація.Tue - 1;
            String Tue1 = Convert.ToString(Авторизація.Tue);

            MySqlCommand comandMon = new MySqlCommand("UPDATE `users` SET `Tuesday` = @hoursMon WHERE `users` . `login` = @loginUser AND `pass` = @passUser;", baza_Mon.GetConnection());
            ...
            comandMon.Parameters.Add("@hoursMon", MySqlDbType.VarChar).Value = Tue1;
