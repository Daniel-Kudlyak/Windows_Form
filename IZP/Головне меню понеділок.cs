...
namespace IZP
{
    public partial class Головне_меню_понеділок : Form
    {
        public Головне_меню_понеділок()
        {
            InitializeComponent();
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command_Mon = new MySqlCommand("SELECT  `Monday` FROM `users` WHERE `login` = @loginUser AND `pass` = @passUser ", baza_Mon.GetConnection());
            ...
            Авторизація.Mon = Convert.ToInt32(res_Mon);

            Авторизація.Mon = Авторизація.Mon - 1;
            String Mon1 = Convert.ToString(Авторизація.Mon);

            MySqlCommand comandMon = new MySqlCommand("UPDATE `users` SET `Monday` = @hoursMon WHERE `users` . `login` = @loginUser AND `pass` = @passUser;", baza_Mon.GetConnection());
            ...
            comandMon.Parameters.Add("@hoursMon", MySqlDbType.VarChar).Value =Mon1;
